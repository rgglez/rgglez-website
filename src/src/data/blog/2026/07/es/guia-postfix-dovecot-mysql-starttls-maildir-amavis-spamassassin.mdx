...
-- Tabla de países permitidos o bloqueados.
...
INSERT INTO geoip_policy (country_iso, action, description, active)
VALUES
  ('MX', 'permit', 'México permitido para SMTP autenticado', 1),
  ('US', 'permit', 'Estados Unidos permitido para SMTP autenticado', 1);
```
...
def geoip_allowed(conn, ip_address):
    """
    Verifica si la IP pertenece a un país permitido.

    Política elegida:
    - Si no se detecta país, se permite.
    - Si hay reglas activas en geoip_policy, sólo se permiten países con action='permit'.
    - Países con action='reject' se rechazan.
    """
...
class PolicyHandler(socketserver.StreamRequestHandler):
    ...
    def handle(self):
        ...
        try:
            ...
            allowed, reason = geoip_allowed(conn, client_address)
            if not allowed:
...
Para DKIM se recomienda usar OpenDKIM o Rspamd. Para DMARC:
...
| `/etc/amavis/conf.d/50-user` | Configuración local Amavis |
...
| `/opt/postfix-policy/policy.py` | Rate limit y GeoIP |
...
- [ ] GeoIP permite y bloquea según `geoip_policy`.
