...
** Vérifier le chemin exact avec `update-java-alternatives --list` en cas de doute.
...
** La propriété `"distribution": "opensearch"` confirme qu'Argilla utilisera le client `opensearch-py` pour se connecter correctement, évitant l'`UnsupportedProductError` qui se produirait avec le client Elasticsearch.
