...
** Verificar la ruta exacta con `update-java-alternatives --list` en caso de duda.
...
** La contraseña debe tener al menos 8 caracteres e incluir mayúsculas, minúsculas, números y algún símbolo especial. De preferencia use algún generador de contraseñas seguras.
...
** La propiedad `"distribution": "opensearch"` confirma que Argilla usará el cliente `opensearch-py` para conectarse correctamente, evitando el `UnsupportedProductError` que ocurriría con el cliente de Elasticsearch.
