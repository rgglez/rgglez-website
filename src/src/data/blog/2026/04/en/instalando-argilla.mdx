...
** Verify the exact path with `update-java-alternatives --list` if in doubt.
...
** The `"distribution": "opensearch"` property confirms that Argilla will use the `opensearch-py` client to connect correctly, avoiding the `UnsupportedProductError` that would occur with the Elasticsearch client.
