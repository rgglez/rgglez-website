...
description: >-
  ...
  heurística y adversarial, STRIPS (Stanford Research Institute Problem Solver)/PDDL/GOAP, HTN, planificación multi-agente
...
- [2. Representación del conocimiento](#2-representacion-del-conocimiento)
  ...
  - [Constraint Satisfaction Problem (CSP) (problema de satisfacción de restricciones)](#constraint-satisfaction-problem-csp-problema-de-satisfaccion-de-restricciones)
  - [Red bayesiana](#red-bayesiana)
  - [Ontología](#ontologia)
  - [Knowledge Graph (grafo de conocimiento)](#knowledge-graph-grafo-de-conocimiento)
  - [Declarative vs. procedural knowledge (conocimiento declarativo vs. procedimental)](#declarative-vs-procedural-knowledge-conocimiento-declarativo-vs-procedimental)
...
- [4. Tipos de lógica](#4-tipos-de-logica)
  ...
  - [First-Order Logic (FOL) (lógica de primer orden)](#first-order-logic-fol-logica-de-primer-orden)
  - [Higher-Order Logic (HOL) (lógica de orden superior)](#higher-order-logic-hol-logica-de-orden-superior)
...
- [5. Búsqueda no informada](#5-busqueda-no-informada)
  ...
  - [Breadth-First Search (BFS) (búsqueda en amplitud)](#breadth-first-search-bfs-busqueda-en-amplitud)
  - [Depth-First Search (DFS) (búsqueda en profundidad)](#depth-first-search-dfs-busqueda-en-profundidad)
  - [Uniform Cost Search (UCS) (búsqueda de costo uniforme)](#uniform-cost-search-ucs-busqueda-de-costo-uniforme)
  - [Iterative Deepening Search (IDS) (búsqueda en profundidad iterativa)](#iterative-deepening-search-ids-busqueda-en-profundidad-iterativa)
...
- [6. Heurística y búsqueda informada](#6-heuristica-y-busqueda-informada)
  ...
  - [Greedy Best-First Search (búsqueda voraz primero el mejor)](#greedy-best-first-search-busqueda-voraz-primero-el-mejor)
  - [A* Search (búsqueda A*)](#a-search-busqueda-a)
  ...
  - [Simulated Annealing (recocido simulado)](#simulated-annealing-recocido-simulado)
...
- [7. Búsqueda adversarial (teoría de juegos)](#7-busqueda-adversarial)
  ...
  - [Monte Carlo Tree Search (MCTS) (búsqueda en árbol Monte Carlo)](#monte-carlo-tree-search-mcts-busqueda-en-arbol-monte-carlo)
- [8. Planificación clásica](#8-planificacion-clasica)
  ...
  - [Efectos](#efectos)
  - [Closed World Assumption (CWA) (hipótesis del mundo cerrado)](#closed-world-assumption-cwa-hipotesis-del-mundo-cerrado)
  ...
  - [Partial-Order Planning (POP) (planificación de orden parcial)](#partial-order-planning-pop-planificacion-de-orden-parcial)
...
- [9. Lenguajes y planificadores](#9-lenguajes-y-planificadores)
  ...
  - [PDDL (Planning Domain Definition Language) (lenguaje de definición de dominios de planificación)](#planning-domain-definition-language-pddl)
  - [ADL (Action Description Language) (lenguaje de descripción de acciones)](#action-description-language-adl)
  - [Fluents](#fluents)
  - [GOAP (Goal-Oriented Action Planning) (planificación orientada a objetivos)](#goal-oriented-action-planning-goap)
  - [FF (Fast-Forward) (planificador heurístico)](#fast-forward-ff)
  - [LAMA](#lama)
  ...
  - [LPG](#lpg)
  - [VAL](#val)
  - [IPC (International Planning Competition) (competencia internacional de planificación)](#international-planning-competition-ipc)
  - [Prolog](#prolog)
  - [LLMs as planners (NL2PDDL) (LLMs como planificadores)](#llms-as-planners-nl2pddl-llms-como-planificadores)
...
- [11. Planificación multi-agente](#11-planificacion-multi-agente)
  - [Multi-Agent System (MAS) (sistema multi-agente)](#multi-agent-system-mas-sistema-multi-agente)
  ...
  - [Planning for Multiple Agents (PTMA) (planificación PARA múltiples agentes)](#planning-for-multiple-agents-ptma-planificacion-para-multiples-agentes)
  - [Planning by Multiple Agents (PBMA) (planificación POR múltiples agentes)](#planning-by-multiple-agents-pbma-planificacion-por-multiples-agentes)
  ...
  - [Distributed planning (planificación distribuida)](#distributed-planning-planificacion-distribuida)
  - [Centralized plan representation (representación centralizada del plan)](#centralized-plan-representation-representacion-centralizada-del-plan)
  - [Shared information (información compartida)](#shared-information-informacion-compartida)
  - [State variables and value domains (variables de estado y dominio de valores)](#state-variables-and-value-domains-variables-de-estado-y-dominio-de-valores)
  - [Multi-functions (multifunciones)](#multi-functions-multifunciones)
  - [Public goals vs. private goals (metas públicas vs. metas privadas)](#public-goals-vs-private-goals-metas-publicas-vs-metas-privadas)
  - [Public actions vs. private actions (acciones públicas vs. acciones privadas)](#public-actions-vs-private-actions-acciones-publicas-vs-acciones-privadas)
  - [Open World Assumption (OWA) (hipótesis de mundo abierto)](#open-world-assumption-hipotesis-de-mundo-abierto-owa)
  - [Plan refinement (refinamiento de planes)](#plan-refinement-refinamiento-de-planes)
  - [Open goal (meta abierta)](#open-goal-meta-abierta)
  - [Threat in multi-agent POP (amenaza en POP multi-agente)](#threat-in-multi-agent-pop-amenaza-en-pop-multi-agente)
  - [Concurrent plan (plan concurrente)](#concurrent-plan-plan-concurrente)
  - [Execution monitoring (monitorización de ejecución)](#execution-monitoring-monitorizacion-de-ejecucion)
  - [Broadcast help request (solicitud de ayuda por broadcast)](#broadcast-help-request-solicitud-de-ayuda-por-broadcast)
  - [MARPE](#marpe)
  - [JADE](#jade)
  - [Reparación de planes multi-agente](#reparacion-de-planes-multi-agente)
  - [Desconocimiento global](#desconocimiento-global)
  - [Decisiones no consensuadas](#decisiones-no-consensuadas)
  - [Tiempos de respuesta](#tiempos-de-respuesta)
  - [Repetición de fallos](#repeticion-de-fallos)
...
- [12. Planificación bajo incertidumbre](#12-planificacion-bajo-incertidumbre)
  ...
  - [POMDP (Partially Observable Markov Decision Process) (proceso de decisión de Markov parcialmente observable)](#partially-observable-markov-decision-process-pomdp)
  - [Hidden Markov Model (HMM) (modelo oculto de Markov)](#hidden-markov-model-hmm-modelo-oculto-de-markov)
  - [Reinforcement Learning (RL) (aprendizaje por refuerzo)](#reinforcement-learning-rl-aprendizaje-por-refuerzo)
...
- [13. Conceptos transversales](#13-conceptos-transversales)
  ...
  - [Agéntica moderna](#agentica-moderna)
...
Arquitectura cognitiva (*Adaptive Control of Thought-Rational*) que modela el
procesamiento humano mediante módulos de memoria declarativa y procedimental.
En IA se usa para estudiar cómo una arquitectura basada en producción de reglas
selecciona acciones bajo restricciones cognitivas.

### SOAR

Arquitectura cognitiva unificada orientada a resolución de problemas mediante
espacios de estados, operadores y aprendizaje por compilación de reglas
(*chunking*). Su hipótesis central es que gran parte del comportamiento
deliberado puede representarse como selección iterativa de operadores.
...
En esta regla, $P$ y $Q$ son proposiciones; $\rightarrow$ significa «implica».
...
En esta regla, $\neg$ significa «no» (negación lógica).
...
En $O(bm)$, $b$ es el factor de ramificación y $m$ la profundidad máxima
explorada.
...
En esta desigualdad, $\leq$ significa «menor o igual que».
...
Aquí, $c(n,a,n')$ es el coste inmediato de aplicar la acción $a$ y pasar de
$n$ a $n'$.
...
Aquí, $g(n)$ es el coste acumulado desde el inicio hasta $n$, $h(n)$ es el
coste estimado desde $n$ a la meta y $f(n)$ es la evaluación total usada para
priorizar nodos.
...
Aquí, $\exp(\cdot)$ es la función exponencial, $\Delta E$ es el empeoramiento
de la solución (incremento de coste) y $T$ controla cuánta exploración se
permite.
...
En la ecuación de minimax, $n$ es el nodo actual, $U(n)$ su utilidad terminal,
$\text{suc}(n)$ el conjunto de sucesores y $\max/\min$ eligen la mejor jugada
para MAX/MIN respectivamente.
...
Aquí, $\text{Pre}(a)$ es el conjunto de precondiciones de la acción $a$,
$S$ el estado actual, $\subseteq$ «subconjunto de» y $\not\subseteq$ «no es
subconjunto de».
...
Cambios que la acción produce en el estado. En STRIPS (Stanford Research Institute Problem Solver) se descomponen en
...
Aquí, $S'$ es el nuevo estado, $\setminus$ elimina elementos de un conjunto y
$\cup$ une conjuntos.
...
asunción estándar en STRIPS (Stanford Research Institute Problem Solver) y PDDL clásico, y simplifica enormemente la
...
- **Enlaces causales** $A \xrightarrow{P} B$: «A produce el fluente $P$ que
  $B$ necesita».
...
**IPC (International Planning Competition) (competencia internacional de planificación)**. Separa la **definición del
...
Extensión de STRIPS (Stanford Research Institute Problem Solver) propuesta por Pednault que admite **disyunciones**,
...
Adaptación de STRIPS (Stanford Research Institute Problem Solver) para entornos dinámicos como videojuegos, ideada por
**Jeff Orkin** (2004) e implementada por primera vez en *F.E.A.R.* (Monolith,
2005). Sus diferencias clave con STRIPS (Stanford Research Institute Problem Solver):
...
Tarea ejecutable directamente por el agente; equivale a un operador STRIPS (Stanford Research Institute Problem Solver).
...
- **Enlace causal** $A \xrightarrow{P} B$: la acción $A$ produce el fluente
...
Arquitectura de planificación en la que varios agentes planifican de forma
descentralizada y cooperan mediante intercambio de información y protocolos de
coordinación, en lugar de depender de un único planificador central.

### Centralized plan representation (representación centralizada del plan)

Esquema donde el artefacto del plan conjunto es compartido por todos los
agentes (por ejemplo, como una «pizarra común»), aunque el proceso de
planificación que lo construye siga siendo distribuido.

### Shared information (información compartida)

Subconjunto de variables, predicados o fluentes que los agentes acuerdan
publicar para coordinarse. Suele declararse explícitamente para equilibrar
cooperación y privacidad del conocimiento local.

### State variables and value domains (variables de estado y dominio de valores)

Formalización del estado del mundo mediante variables $V$, cada una con un
dominio finito de valores mutuamente excluyentes $D_v$. Un **fluente** es una
instanciación $\langle v, d \rangle$ de una variable con uno de sus valores.

### Multi-functions (multifunciones)

Mecanismo de modelado para codificar información del problema de forma compacta
en dominios multi-agente, facilitando la representación de conocimiento parcial
y la manipulación de variables instanciadas compartidas.

### Public goals vs. private goals (metas públicas vs. metas privadas)

- **Metas públicas:** objetivos globales que deben ser satisfechos de manera
  consistente por el plan conjunto.
- **Metas privadas:** objetivos internos de cada agente, resueltos localmente
  sin exponer necesariamente toda su información o estrategia.

### Public actions vs. private actions (acciones públicas vs. acciones privadas)

- **Acciones públicas:** operadores conocidos por varios agentes, con
  precondiciones y efectos compartidos para coordinar dependencias.
- **Acciones privadas:** operadores locales de un agente, no totalmente
  observables por otros, que preservan autonomía y privacidad.

### Open World Assumption (OWA) (hipótesis de mundo abierto)

Supuesto en el que la información no especificada explícitamente se considera
**desconocida** y no falsa por defecto. En multi-agente modela mejor la visión
parcial y heterogénea del entorno.

### Plan refinement (refinamiento de planes)

Proceso iterativo donde los agentes parten de un plan base (a menudo parcial o
vacío) y agregan subplanes para resolver metas abiertas, manteniendo
consistencia causal y de orden.

### Open goal (meta abierta)

Precondición o condición objetivo del plan parcial que todavía no está
soportada por ninguna acción válida del plan actual, por lo que requiere
refinamiento adicional.

### Threat in multi-agent POP (amenaza en POP multi-agente)

Situación en la que una acción puede invalidar un enlace causal existente entre
otras dos acciones del plan parcial. Se resuelve con restricciones de orden,
cambios de refinamiento o reasignación de acciones.

### Concurrent plan (plan concurrente)

Plan en el que múltiples agentes ejecutan acciones en paralelo cuando son
mutuamente consistentes y no generan conflictos sobre recursos, precondiciones
o efectos.

### Execution monitoring (monitorización de ejecución)

Supervisión continua del estado y de la ejecución de acciones para detectar
desviaciones, fallos o violaciones de precondiciones y activar reparación o
replanificación.

### Broadcast help request (solicitud de ayuda por broadcast)

Patrón de comunicación donde un agente difunde a varios colaboradores una
petición de apoyo ante un fallo, para recibir alternativas de reparación y
seleccionar la más conveniente.

### MARPE

Marco de trabajo de planificación y ejecución reactiva en entornos
multi-agente, orientado a detectar fallos durante la ejecución y coordinar
reparación
distribuida en tiempos cortos.

### JADE

**Java Agent DEvelopment Framework**, plataforma para implementar sistemas
multi-agente en Java siguiendo estándares FIPA (mensajería ACL, directorio de
servicios y ciclos de vida de agentes). Se usa para prototipar coordinación,
negociación y ejecución distribuida.

### Reparación de planes multi-agente

Proceso de ajustar un plan conjunto cuando cambian el entorno, los recursos o
la disponibilidad de agentes, evitando replanificar todo desde cero. Busca
recuperar factibilidad con cambios locales y bajo coste de coordinación.

### Desconocimiento global

Situación en la que ningún agente posee una visión completa y actualizada del
estado global del sistema. Obliga a coordinarse con información parcial,
incrementando la incertidumbre y el riesgo de conflictos entre planes.

### Decisiones no consensuadas

Decisiones tomadas por uno o varios agentes sin acuerdo explícito del resto
cuando el protocolo requería consenso o validación compartida. Pueden acelerar
la reacción local, pero también introducir incoherencias y reprocesos.

### Tiempos de respuesta

Latencia entre la detección de un evento y la emisión/ejecución de una acción
coordinada por el sistema multi-agente. Depende de cómputo local, comunicación,
sincronización y complejidad del protocolo de coordinación.

### Repetición de fallos

Recurrencia del mismo patrón de error en ciclos sucesivos de coordinación o
ejecución (por ejemplo, conflicto de recursos no resuelto). Indica debilidad
en diagnóstico, aprendizaje o reglas de recuperación del sistema.

### Roles de agentes

Especialización funcional de los agentes dentro del sistema (p. ej.,
coordinador, planificador, ejecutor, monitor, negociador). Definir roles
claros reduce ambiguedad, mejora escalabilidad y facilita protocolos de
interacción robustos.
...
Aquí, $V^*(s)$ es el valor óptimo del estado $s$; $\max_a$ elige la mejor
acción; $R(s,a)$ es recompensa inmediata; $\gamma$ descuenta recompensas
futuras; $P(t \mid s,a)$ es probabilidad de llegar al estado $t$ desde $s$ con
$a$; y $\sum_t$ agrega el valor esperado sobre todos los estados siguientes.
...
En IA aplicada (2024-2026), **agéntica moderna** se refiere al diseño de
sistemas basados en LLMs que no solo responden texto, sino que **perciben
estado, planifican, usan herramientas, ejecutan acciones y replanifican**.
Suele modelarse como un bucle:

1. Interpretar objetivo y contexto.
2. Proponer un plan (explícito o implícito).
3. Llamar herramientas (búsqueda, código, bases de datos, APIs).
4. Verificar resultados.
5. Actualizar memoria/estado y decidir siguiente paso.

Conceptualmente, conecta con BDI (creencias/objetivos/intenciones), con
planificación reactiva y con arquitecturas híbridas deliberativo-reactivas.

### MCP (Model Context Protocol)

**MCP** es un protocolo abierto para estandarizar cómo un modelo o agente
consume contexto externo y herramientas. En vez de integrar cada fuente de
datos con adaptadores ad hoc, MCP define una interfaz común para exponer:
...
En términos de planificación, MCP reduce el coste de integración del entorno
del agente y facilita arquitecturas modulares multi-herramienta, mejorando
trazabilidad, gobernanza y reproducibilidad de ejecuciones agénticas.
...
Aquí, $S$ denota al sujeto cognoscente y $p$ al enunciado/proposición.
...
STRIPS (Stanford Research Institute Problem Solver) capturan condiciones esenciales para la acción.
...
En esa notación, $\square\!\!\!\rightarrow$ representa la implicación
contrafáctica («si fuera el caso que..., entonces...»), distinta de la
implicación material $\rightarrow$ de la lógica clásica.
...
- Fikes, R. y Nilsson, N. (1971). *STRIPS (Stanford Research Institute Problem Solver): A New Approach to the Application
