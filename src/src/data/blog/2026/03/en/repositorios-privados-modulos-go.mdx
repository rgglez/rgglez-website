...
First let's define some assumptions:
...
runs the steps on their own machine. Use of a modern Linux
distribution is also assumed. In my case, Debian 13.
...
Both fail with private repos, so they need to be excluded:
...
| Variable | Effect |
|---|---|
| `GOPRIVATE` | Tells Go that `github.com/myorg/*` is private. Implies `GONOSUMDB` and `GONOPROXY`. |
...
# All three should show github.com/myorg/*
```
...
|---|---|---|
| `410 Gone` on `go get` | Go tries to use `proxy.golang.org` | Verify that `GOPRIVATE` includes `github.com/myorg/*` |
