...
- [Flow E — Deploy key for Go only + HTTPS for development](#flow-e--deploy-key-for-go-only--https-for-development)
- [E1. Packages](#e1-packages)
- [E2. Generate the deploy key](#e2-generate-the-deploy-key)
- [E3. Register the deploy key as read-only](#e3-register-the-deploy-key-as-read-only)
- [E4. Alias in `~/.ssh/config`](#e4-alias-in-sshconfig)
- [E5. Configure `ssh-agent`](#e5-configure-ssh-agent)
- [E6. Personal Access Token + Credential Helper for HTTPS](#e6-personal-access-token--credential-helper-for-https)
- [E7. Go wrapper with `GIT_CONFIG_*`](#e7-go-wrapper-with-git_config_)
- [E8. Go variables for private modules](#e8-go-variables-for-private-modules)
- [E9. Verify](#e9-verify)
- [E10. Daily workflow](#e10-daily-workflow)
- [E11. Publish a new version](#e11-publish-a-new-version)
- [E12. Quick summary (all in one block)](#e12-quick-summary-all-in-one-block)
- [E13. Troubleshooting](#e13-troubleshooting)
...
This flow achieves **total separation** between Go and git. Go uses SSH with the
deploy key to download private modules. The developer uses HTTPS with
their PAT for all git work (clone, pull, push). Neither interferes
with the other.
...
Verify that you have Git ≥ 2.31, which is the version that introduced support
...
Same as in Flows C and D: a dedicated key with a name that identifies
the repo.
...
> **GitHub restriction**: the same public key cannot be a deploy key
> for more than one repository. Generate a separate pair for each repo that
> needs one.
...
The only thing Go needs to do with the module is download it. It does not need
to write. That is why the deploy key is registered as read-only (minimum
privilege):
...
The alias works exactly as in Flows C and D. See the detailed explanation
of each directive in step C4.
...
The developer uses HTTPS for all git work. The PAT and the credential helper
are configured exactly as in Flow A.
...
Remember that the credential helper stores **one credential per host**, so
...
Git 2.31+ recognizes three environment variables that allow injecting
ad-hoc configuration entries:

- `GIT_CONFIG_COUNT=N` — how many entries there are
- `GIT_CONFIG_KEY_0`, `GIT_CONFIG_KEY_1`, ... — the key of each entry
- `GIT_CONFIG_VALUE_0`, `GIT_CONFIG_VALUE_1`, ... — the value of each entry
...
3. The function sets `GIT_CONFIG_COUNT=1`, `GIT_CONFIG_KEY_0`, and
   `GIT_CONFIG_VALUE_0` as environment variables of the process.
...
8. The entry says: "rewrite `https://github.com/myorg/my-go-module` to
   `git@github-my-go-module:myorg/my-go-module`".
...
If it says `go is /usr/bin/go` or similar, the wrapper was not loaded. Run
`source ~/.bashrc`.

> **Multiple private modules with deploy key**: increment `GIT_CONFIG_COUNT` and
> add additional `KEY_N`/`VALUE_N` pairs. Each module needs its own
> deploy key, SSH alias, and pair of variables:
> ```bash
> go() {
>     GIT_CONFIG_COUNT=2 \
>     GIT_CONFIG_KEY_0="url.git@github-my-go-module:myorg/my-go-module.insteadOf" \
>     GIT_CONFIG_VALUE_0="https://github.com/myorg/my-go-module" \
>     GIT_CONFIG_KEY_1="url.git@github-another-module:myorg/another-module.insteadOf" \
>     GIT_CONFIG_VALUE_1="https://github.com/myorg/another-module" \
>     command go "$@"
> }
> ```
...
The same reasons as always: Go must not consult the public proxy or the
checksum database for private modules.
...
> export GOPRIVATE="github.com/myorg/my-go-module,github.com/myorg/another-module"
> ```
> Repeat the same pattern in `GONOSUMDB` and `GONOSUMCHECK`.

---

## E9. Verify

Verification has two independent parts: confirm that Go uses the deploy key,
and confirm that git uses HTTPS with the PAT.
...
The check with `git remote -v` is important: if the URLs show
...
The developer uses HTTPS as usual. No need to know that Go uses a
different channel. Everything is transparent:
