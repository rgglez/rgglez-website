...
```mermaid
flowchart TD
    DEV["Developer<br/>go get ..."] --> GO["Go command<br/>(resolve module and version)"]
    GO -->|"queries"| PROXY["proxy.golang.org<br/>(cached zip/mod/info)"]
    PROXY -->|"verifies checksums"| SUM["sum.golang.org<br/>(transparency and integrity)"]
    SUM -->|"downloads module"| CACHE[("Local module cache<br/>$GOMODCACHE")]
    CACHE -->|"updates"| FILES[("go.mod / go.sum")]
```
