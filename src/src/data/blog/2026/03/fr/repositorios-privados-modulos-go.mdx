...
```mermaid
flowchart TD
    DEV["Développeur<br/>go get ..."] --> GO["Commande Go<br/>(résoudre le module et la version)"]
    GO -->|"requête"| PROXY["proxy.golang.org<br/>(zip/mod/info en cache)"]
    PROXY -->|"vérifie les checksums"| SUM["sum.golang.org<br/>(transparence et intégrité)"]
    SUM -->|"télécharge le module"| CACHE[("Cache local des modules<br/>$GOMODCACHE")]
    CACHE -->|"met à jour"| FILES[("go.mod / go.sum")]
```
