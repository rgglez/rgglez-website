...
```mermaid
flowchart TD
    DEV["Desarrollador<br/>go get ..."] --> GO["Comando Go<br/>(resolver módulo y versión)"]
    GO -->|"consulta"| PROXY["proxy.golang.org<br/>(zip/mod/info cacheados)"]
    PROXY -->|"verifica checksums"| SUM["sum.golang.org<br/>(transparencia e integridad)"]
    SUM -->|"descarga módulo"| CACHE[("Cache local de módulos<br/>$GOMODCACHE")]
    CACHE -->|"actualiza"| FILES[("go.mod / go.sum")]
```
